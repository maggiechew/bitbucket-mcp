...
- **discoverJenkinsPipeline** — samples recent completed builds and describes the pipeline as observed: stage names and which run in parallel, outcome counts, how often a test report is published, and the failure signatures seen with example console lines. Given a multibranch folder (`job`, default the repo slug) it samples the latest completed build of each branch job; given a standalone job such as a master or nightly job it samples that job's last `sample_size` builds, and `kind` says which it found. Meant for building and refreshing a local manifest of how a given Jenkins behaves, rather than hardcoding one pipeline's shape
...
The PR's Jenkins job is read from the URL in its Bitbucket build status, so the folder can be named anything. When a PR has no status yet, the multibranch convention `<repo slug>/PR-<id>` is tried instead. `discoverJenkinsPipeline` defaults its `job` to the repo slug; pass it when your folder is named differently, or to profile a standalone job. `getJenkinsBuildReport` and `listJenkinsBuilds` take a job name directly, so a master or nightly job that lives outside the multibranch folder is reachable by its own name.
