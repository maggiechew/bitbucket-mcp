...
export function registerDiffTools(server: McpServer): void {
  server.tool(
    ...
    async ({ workspace, repo_slug, pull_request_id }) => {
      ...
      const result = await bitbucketRequest<string>(
        `/repositories/${ctx.workspace}/${ctx.repoSlug}/pullrequests/${pull_request_id}/diff`,
        { accept: "text/plain" },
      );
  ...
  server.tool(
    ...
    async ({ workspace, repo_slug, pull_request_id, page, pagelen }) => {
      ...
      const path = `/repositories/${ctx.workspace}/${ctx.repoSlug}/pullrequests/${pull_request_id}/diffstat${query ? `?${query}` : ""}`;
